...
public class Json {
	...
	public void toJson(@Nullable Object object, WriteableFileHandle file) {
		toJson(object, object == null ? null : object.getClass(), null, file);
	}

	/** @param knownType May be null if the type is unknown. */
	public void toJson(@Nullable Object object, @Nullable Class knownType, WriteableFileHandle file) {
	...
	public void toJson(
			...
			@Nullable Class knownType, @Nullable Class<?> elementType, WriteableFileHandle file) {
		Writer writer = null;
		try {
	 ...
	 */
	public void toJson(
			@Nullable Object object, @Nullable Class knownType, @Nullable Class<?> elementType, Writer writer) {
		setWriter(writer);
		try {
	...
	public void writeValue(String name,
	                       @Nullable Object value, @Nullable Class knownType, @Nullable Class<?> elementType) {
		try {
			writer.name(name);
		} catch(IOException ex) {
			throw new SerializationException(ex);
	...
	public void readField(Object object, String fieldName, String jsonName,
	                      @Nullable Class<?> elementType, JsonValue jsonMap) {
		Class<?> type = object.getClass();
		FieldMetadata metadata = getFields(type).get(fieldName);
		if(metadata == null)
			throw new SerializationException("Field not found: " + fieldName + " (" + type.getName() + ")");
	...
	public void readField(
			@Nullable Object object, Field field, String jsonName, @Nullable Class<?> elementType, JsonValue jsonMap) {
	 ...
	 * @return May be null.
	 */
	public @Nullable <T> T readValue(String name,
	                                 @Nullable Class<T> type, @Nullable Class<?> elementType, JsonValue jsonMap) {
	...
	public @Nullable <T> T readValue(String name,
	                                 ...
	                                 @Nullable Class<?> elementType, T defaultValue, JsonValue jsonMap) {
		JsonValue jsonValue = jsonMap.get(name);
	 ...
	 * @return May be null.
	 */
	public @Nullable <T> T readValue(
			@Nullable Class<T> type, @Nullable Class<?> elementType, T defaultValue, JsonValue jsonData) {
	...
	public @Nullable <T> T readValue(@Nullable Class<T> type, @Nullable Class<?> elementType, JsonValue jsonData) {
		...
		if(jsonData.isObject()) {
			...
			if(className != null) {
				type = (Class<T>) getClass(className);
				if(type == null) {
					try {
			...
			if(typeName != null && Collection.class.isAssignableFrom(type)) {
				...
				// JSON object special cases.
				if(object instanceof ObjectMap result) {
	...
	static private class FieldMetadata {
		...
		public FieldMetadata(Field field) {
			this.field = field;
			int index = (ObjectMap.class.isAssignableFrom(field.getType())
					|| Map.class.isAssignableFrom(field.getType())) ? 1 : 0;
			this.elementType = getElementType(index);
			deprecated = field.isAnnotationPresent(Deprecated.class);
		}

		private Class<?> getElementType(int index) {
			...
			if(genericType instanceof ParameterizedType) {
				...
				if(actualTypes.length - 1 >= index) {
					...
					if(actualType instanceof Class<?> clazz)
						return clazz;
					else if(actualType instanceof ParameterizedType tyoe)
						return (Class<?>) tyoe.getRawType();
