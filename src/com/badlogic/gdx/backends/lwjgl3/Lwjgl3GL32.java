...
public class Lwjgl3GL32 extends Lwjgl3GL31 implements GL32 {
	...
	public void glDrawElementsBaseVertex(int mode, int count, int type, Buffer indices, int basevertex) {
		if(indices instanceof ShortBuffer sb && type == GL20.GL_UNSIGNED_SHORT) {
			int position = sb.position();
			int oldLimit = sb.limit();
			sb.limit(position + count);
			GLES32.glDrawElementsBaseVertex(mode, sb, basevertex);
			sb.limit(oldLimit);
			...
			GLES32.glDrawElementsBaseVertex(mode, sb, basevertex);
			sb.limit(oldLimit);
		} else if(indices instanceof ByteBuffer bb && type == GL20.GL_UNSIGNED_BYTE) {
			int position = bb.position();
			int oldLimit = bb.limit();
			bb.limit(position + count);
			GLES32.glDrawElementsBaseVertex(mode, bb, basevertex);
			bb.limit(oldLimit);
		} else
	...
	public void glDrawRangeElementsBaseVertex(int mode, int start, int end, int count, int type, Buffer indices, int basevertex) {
		if(indices instanceof ShortBuffer sb && type == GL20.GL_UNSIGNED_SHORT) {
			...
			int oldLimit = sb.limit();
			sb.limit(position + count);
			GLES32.glDrawRangeElementsBaseVertex(mode, start, end, sb, basevertex);
			sb.limit(oldLimit);
		} else if(indices instanceof ByteBuffer && type == GL20.GL_UNSIGNED_SHORT) {
			ShortBuffer sb = ((ByteBuffer) indices).asShortBuffer();
			int position = sb.position();
			...
			int oldLimit = bb.limit();
			bb.limit(position + count);
			GLES32.glDrawRangeElementsBaseVertex(mode, start, end, bb, basevertex);
			bb.limit(oldLimit);
		} else
			throw new GdxRuntimeException(
					"Can't use " + indices.getClass().getName() + " with this method. Use ShortBuffer or ByteBuffer instead.");
	...
	public void glDrawElementsInstancedBaseVertex(int mode, int count, int type, Buffer indices, int instanceCount,
		...
		if(indices instanceof ShortBuffer sb && type == GL20.GL_UNSIGNED_SHORT) {
			...
			int oldLimit = sb.limit();
			sb.limit(position + count);
			GLES32.glDrawElementsInstancedBaseVertex(mode, sb, instanceCount, basevertex);
			sb.limit(oldLimit);
		} else if(indices instanceof ByteBuffer && type == GL20.GL_UNSIGNED_SHORT) {
			ShortBuffer sb = ((ByteBuffer) indices).asShortBuffer();
			int position = sb.position();
			...
			int oldLimit = bb.limit();
			bb.limit(position + count);
			GLES32.glDrawElementsInstancedBaseVertex(mode, bb, instanceCount, basevertex);
			bb.limit(oldLimit);
		} else
			throw new GdxRuntimeException(
					"Can't use " + indices.getClass().getName() + " with this method. Use ShortBuffer or ByteBuffer instead.");
