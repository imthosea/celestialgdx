	...
	protected void build() {
		...
		if(bufferBuilder.hasDepthRenderBuffer) {
			...
			if(bufferBuilder.samples > 0) {
				GL33.glRenderbufferStorageMultisample(GL20.GL_RENDERBUFFER, bufferBuilder.samples,
		...
		for(FrameBufferRenderBufferAttachmentSpec colorBufferSpec : bufferBuilder.colorRenderBufferSpecs) {
			...
			if(bufferBuilder.samples > 0) {
				GL33.glRenderbufferStorageMultisample(GL20.GL_RENDERBUFFER, bufferBuilder.samples, colorBufferSpec.internalFormat,
						width, height);
			} else {
		...
		if(result == GL20.GL_FRAMEBUFFER_UNSUPPORTED && bufferBuilder.hasDepthRenderBuffer && bufferBuilder.hasStencilRenderBuffer
			...
			gl.glBindRenderbuffer(GL20.GL_RENDERBUFFER, depthStencilPackedBufferHandle);
			if(bufferBuilder.samples > 0) {
				GL33.glRenderbufferStorageMultisample(GL20.GL_RENDERBUFFER, bufferBuilder.samples, GL_DEPTH24_STENCIL8_OES, width,
						height);
			} else {
		...
		if(result != GL20.GL_FRAMEBUFFER_COMPLETE) {
			...
			// celestialgdx: swapped to LWJGL
			if(result == GL33.GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE)
